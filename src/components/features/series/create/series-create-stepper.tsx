...
export interface SeriesCreateStepperProps {
  currentStep: number;
...
/**
 * Series Create Stepper Component
 * Visual timeline showing completed/current/pending states
 * Supports clicking on completed steps to jump back
 */
export function SeriesCreateStepper({
  currentStep,
  completedSteps,
  onStepClick,
  className,
}: SeriesCreateStepperProps) {
  const { t } = useI18n();

  const stepLabels = [
    t("create.steps.selection", "series"), // Step 0 - Selection
    t("create.steps.basic", "series"),
    t("create.steps.media", "series"),
    t("create.steps.content", "series"),
    t("create.steps.release", "series"),
    t("create.steps.advanced", "series"),
  ];
    ...
    <div className={cn("w-full overflow-x-auto pb-2", className)}>
      ...
      <div className="hidden md:flex items-start w-full relative py-4">
        {Array.from({ length: TOTAL_STEPS }, (_, i) => {
          const step = i; // Start from 0
          const state = getStepState(step);
          const isClickable = completedSteps.includes(step);
          const isLastStep = step === TOTAL_STEPS - 1; // Last step is TOTAL_STEPS - 1 (step 5)
            ...
            >
              ...
              <div className="flex flex-col items-center relative z-10 w-full">
                <button
                  ...
                  aria-label={`Step ${step}: ${stepLabels[step]}`}
                >
                  {state === "completed" ? (
