...
import type { CreateStudioDto } from "@/lib/interface/studio.interface";
...
interface CreateStudioFormDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (data: CreateStudioDto) => Promise<void>;
  isLoading?: boolean;
}

export function CreateStudioFormDialog({
  open,
  onOpenChange,
  onSubmit,
  isLoading,
}: CreateStudioFormDialogProps) {
  const { t } = useI18n();
  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      type: undefined,
      siteUrl: "",
      status: STUDIO_CONSTANTS.STATUS.ACTIVE,
    },
  });

  const handleSubmit = async (data: StudioFormValues) => {
    const submitData: CreateStudioDto = {
      name: data.name,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>{t("studios.form.createTitle", "admin")}</DialogTitle>
          <DialogDescription>
            {t("studios.form.createDescription", "admin")}
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form
