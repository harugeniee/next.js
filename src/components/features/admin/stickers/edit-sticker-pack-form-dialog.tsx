...
interface EditStickerPackFormDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  pack?: StickerPack;
  onSubmit: (data: UpdateStickerPackDto) => Promise<void>;
  isLoading?: boolean;
}

export function EditStickerPackFormDialog({
  open,
  onOpenChange,
  pack,
  onSubmit,
  isLoading,
}: EditStickerPackFormDialogProps) {
  const { t } = useI18n();
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>{t("stickers.form.editPackTitle", "admin")}</DialogTitle>
          <DialogDescription>
            {t("stickers.form.editPackDescription", "admin")}
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form
