...
export interface MenuDockProps {
  ...
  showLabels?: boolean;
  animated?: boolean;
  activeIndex?: number; // Optional external control of active index
  onActiveIndexChange?: (index: number) => void; // Callback when active index changes
}
...
export const MenuDock: React.FC<MenuDockProps> = ({
  ...
  // Update internal state when external activeIndex changes
  useEffect(() => {
    if (externalActiveIndex !== undefined) {
      if (externalActiveIndex >= 0 && externalActiveIndex < finalItems.length) {
        // External control - don't update internal state
        return;
      }
    }
  }, [externalActiveIndex, finalItems.length]);
